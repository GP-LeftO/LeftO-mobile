...
export default function RoleSpecificInfoScreen({
  ...
  const { uploadDocument, registerSeller } = useAuth();
  ...
  const handleSubmit = async () => {
    ...
    // Account already registered in BasicInfoScreen — go straight to role-specific step
    if (role === "buyer") {
  ...
  const uploadAndFinalize = async () => {
    ...
    if (docUri) {
      ...
      try {
        ...
        const fileUrl = await uploadDocument(docUri, docType, (pct) => setUploadProgress(pct));
    ...
    try {
      ...
      setSubmitError(msg ?? (rtl ? "فشل إرسال طلب التسجيل، يرجى المحاولة مجدداً" : "Registration submission failed, please try again"));
    } finally {
      setIsSubmitting(false);
    }
  };
      ...
      <KeyboardAvoidingView style={styles.keyboardView} behavior={Platform.OS === "ios" ? "padding" : undefined}>
        ...
        >
          <View style={styles.headerRow}>
            ...
            <View style={styles.stepWrap}><StepIndicator current={5} total={5} /></View>
          </View>
          ...
          {role === "buyer" && (
            <Animated.View entering={FadeInDown.delay(180).duration(500).springify()} style={styles.section}>
              <Text style={styles.sectionLabel}><Feather name="map-pin" size={14} color={Colors.primaryOrange} /> {rtl ? "موقعك" : "Your location"}</Text>
              {pickedLocation ? (
                ...
                <TouchableOpacity style={[styles.mapPickerBtn, !!errors.location && styles.mapPickerBtnError]} onPress={() => { setShowMap(true); setErrors(e => ({ ...e, location: "" })); }} activeOpacity={0.85}>
            ...
            <Animated.View entering={FadeInDown.delay(180).duration(500).springify()} style={styles.section}>
              ...
              <View style={styles.pillsWrap}>
                {BUSINESS_TYPES.map(bt => (
                  <TouchableOpacity key={bt.value} style={[styles.pill, businessType === bt.value && styles.pillActive]} onPress={() => { setBusinessType(bt.value); setErrors(e => ({ ...e, btype: "" })); }}>
          ...
          <Animated.View entering={FadeInDown.delay(480).duration(500).springify()} style={{ gap: 12 }}>
            {uploadFailed ? (
              <Button label={rtl ? "إعادة رفع الوثيقة" : "Retry upload"} onPress={uploadAndFinalize} variant="primary" loading={isSubmitting && !uploadStep} testID="role-specific-retry-upload" />
            ) : (
              <Button
                label={rtl ? (role === "buyer" ? "الذهاب للرئيسية" : "إرسال الطلب") : (role === "buyer" ? "Go to Home" : "Submit Application")}
            ...
            {uploadStep && (
              <View style={styles.progressWrap}>
                <Text style={[styles.progressLabel, rtl && styles.rtl]}>{rtl ? "جاري رفع الوثيقة…" : "Uploading document…"}</Text>
...
const styles = StyleSheet.create({
  ...
  section: { gap: 10 },
  sectionLabel: { fontSize: 14, fontWeight: "600", color: Colors.grayDark },
  sectionHint: { fontSize: 12, color: Colors.grayMedium, marginTop: -4 },
  pillsWrap: { flexDirection: "row", flexWrap: "wrap", gap: 8 },
  pill: { paddingHorizontal: 14, paddingVertical: 8, borderRadius: 20, borderWidth: 1.5, borderColor: Colors.grayLight, backgroundColor: Colors.white },
  pillWide: { paddingHorizontal: 16 },
  pillActive: { backgroundColor: Colors.primaryOrange, borderColor: Colors.primaryOrange },
  pillText: { fontSize: 13, color: Colors.grayDark, fontWeight: "500" },
  pillTextActive: { color: Colors.white },
  mapPickerBtn: {
    borderWidth: 1.5, borderColor: Colors.grayLight, borderRadius: 16,
    backgroundColor: Colors.white, padding: 20, alignItems: "center", gap: 8,
  },
  mapPickerBtnError: { borderColor: "#ef4444" },
  mapPickerBtnInner: { position: "relative", width: 48, height: 48, alignItems: "center", justifyContent: "center" },
  mapPreviewDot: { position: "absolute", width: 48, height: 48, borderRadius: 24, backgroundColor: Colors.orangeLight },
  mapPickerBtnLabel: { fontSize: 15, fontWeight: "700", color: Colors.grayDark },
  mapPickerBtnSub: { fontSize: 12, color: Colors.grayMedium, textAlign: "center" },
  locationSelected: {
    flexDirection: "row", alignItems: "center", gap: 12,
    backgroundColor: Colors.white, borderRadius: 14, borderWidth: 1.5, borderColor: Colors.primaryOrange, padding: 14,
  },
  locationSelectedIcon: { width: 36, height: 36, borderRadius: 18, backgroundColor: Colors.orangeLight, alignItems: "center", justifyContent: "center" },
  locationSelectedText: { flex: 1 },
  locationSelectedAddress: { fontSize: 13, fontWeight: "600", color: Colors.grayDark },
  locationSelectedChange: { fontSize: 11, color: Colors.primaryOrange, marginTop: 2 },
  textAreaWrap: {
    backgroundColor: Colors.white, borderRadius: 14, borderWidth: 1.5, borderColor: Colors.grayLight,
    paddingHorizontal: Spacing.md, paddingVertical: 12, minHeight: 100,
  },
  inputError: { borderColor: "#ef4444" },
  textArea: { fontSize: 14, color: Colors.grayDark, lineHeight: 20 },
  uploadBtn: {
    flexDirection: "row", alignItems: "center", gap: 10,
    backgroundColor: Colors.white, borderRadius: 14, borderWidth: 1.5,
    borderColor: Colors.grayLight, borderStyle: "dashed", padding: 16,
  },
  uploadBtnDone: { borderColor: Colors.greenMain, borderStyle: "solid", backgroundColor: "#f0fdf4" },
  uploadText: { flex: 1, fontSize: 14, color: Colors.grayMedium },
  uploadTextDone: { color: Colors.greenMain, fontWeight: "600" },
  approvalNote: { flexDirection: "row", gap: 8, backgroundColor: Colors.orangeLight, borderRadius: 12, padding: 12, alignItems: "flex-start" },
  approvalNoteText: { flex: 1, fontSize: 13, color: Colors.primaryOrange, lineHeight: 18 },
  apiErrorBox: { flexDirection: "row", gap: 8, backgroundColor: "#fef2f2", borderRadius: 12, padding: 12, alignItems: "flex-start" },
  apiErrorText: { flex: 1, fontSize: 13, color: "#ef4444", lineHeight: 18 },
  errorText: { fontSize: 12, color: "#ef4444", marginTop: 2 },
  progressWrap: { gap: 6 },
  progressLabel: { fontSize: 13, color: Colors.grayDark, fontWeight: "500" },
  progressTrack: { height: 6, backgroundColor: Colors.grayLight, borderRadius: 3, overflow: "hidden" },
  progressFill: { height: 6, backgroundColor: Colors.primaryOrange, borderRadius: 3 },
  progressPct: { fontSize: 11, color: Colors.grayMedium },
});